...
Ext.override(Ext.grid.GridPanel, {
    ...
    createWorksheet: function(includeHidden,config) {
        ...
        for (var i = 0; i < colCount; i++) {
            if ((cm.getDataIndex(i) != '')
                ...
                else
                {
                    ...
                    var fld = config.store.recordType.prototype.fields.get(cm.getDataIndex(i));
                    switch(fld.type) {
                        case "int":
        ...
        var t = '<ss:Worksheet ss:Name="' + this.title + '">' +
            ...
            '" ss:ExpandedRowCount="' + (config.store.getCount() + 2) + '">' +
            colXml +
            '<ss:Row ss:Height="38">' +
        ...
        for (var i = 0, it = config.store.data.items, l = it.length; i < l; i++) {
            t += '<ss:Row>';
